...
def _build_status(
    *,
    ...
    if tmux_status_segments is not None and snapshot is not None:
        plain_left, plain_ip = tmux_status_segments(snapshot, mode)
        ip = plain_ip or ip
        bits = plain_left.split()
        colored: list[str] = []
        for bit in bits:
            if "/" in bit:
                num, den = bit.split("/", 1)
                if num.isdigit() and den.isdigit():
                    colored.append(f"#[fg=green]{bit}#[default]")
                    continue
            elif bit.startswith(("VEC:", "PBX:")):
                colored.append(_ok_tmux(bit))
            elif bit.startswith("WAN:"):
                glyph = bit.split(":", 1)[-1]
                wan_fg = "green" if glyph == "1" else ("yellow" if glyph == "0" else "red")
                colored.append(f"#[fg={wan_fg}]WAN:{glyph}#[default]")
            else:
                colored.append(f"#[fg=cyan]{bit}#[default]")
        left = " ".join(colored)
        right = f"#[fg=green]{ip or host}#[fg=yellow] {TMUX_TIME_FMT}"
        return left, right

    right = f"#[fg=green]{host} {ip} #[fg=yellow]{TMUX_TIME_FMT}"
    return f"#[fg=cyan]{host}#[default]", right
